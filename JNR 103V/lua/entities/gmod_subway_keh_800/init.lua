...
function ENT:Initialize()
	...
	self:SetModel("models/train/JNR/keihan_800_ct.mdl")
	self.BaseClass.Initialize(self)
	self:SetPos(self:GetPos() + Vector(0,0,140))
	
	-- Create seat entities
	self.DriverSeat = self:CreateSeat("driver",Vector(340,25,65)) 
	self.InstructorsSeat = self:CreateSeat("instructor",Vector(340,-25,65))

	-- Hide seats
	self.DriverSeat:SetColor(Color(255,255,255,255))
	self.DriverSeat:SetRenderMode(RENDERMODE_TRANSALPHA)
	self.InstructorsSeat:SetColor(Color(255,255,255,255))
	self.InstructorsSeat:SetRenderMode(RENDERMODE_TRANSALPHA)

	
	-- Create Bogeys
	self.FrontBogey = self:CreateBogey(Vector( 395-120,0,18),Angle(0,180,0),true,"tatra") 
	self.RearBogey = self:CreateBogey(Vector(-455+170,0,18),Angle(0,0,0),false,"tatra")
